...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        labels, shapes, self.segments,value = zip(*cache.values())
        ...
        if self.rect:
            ...
            self.value=[self.value[i] for i in irect]
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix='', kpt_label=False):
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            ...
                            for i in range(len(l)):
                                ...
                                kptv = l[i,5:][np.arange(2, l.shape[1]-5, 3)]
                                kptvs[i]=kptv#np.hstack((l[i,:5],kptv))
                ...
                x[im_file] = [l, shape, segments,value]
            except Exception as e:
                nc += 1
    ...
    def __getitem__(self, index):
        ...
        value_out=torch.zeros((nL,1+num_kpts)) if self.kpt_label else torch.zeros((nL,6))


        if nL:
            if  self.kpt_label:
                labels_out[:, 1:] = torch.from_numpy(labels)
                value_out[:,1:] =torch.from_numpy(values)
            else:
                labels_out[:, 1:] = torch.from_numpy(labels[:, :5])
        """
        print("++++++++分割1111++++++++")
        print("标签11111:", labels_out.size())
        print(value_out.size())
        if value_out.size()[0] != labels_out.size()[0]:
            print("warning!!!")
        print("++++++++++++++++++++")
        """
        ...
        return torch.from_numpy(img), labels_out, self.img_files[index], shapes , value_out

    @staticmethod
    def collate_fn(batch):
        img, label, path, shapes,value = zip(*batch)  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        for k,f in enumerate(value):
            f[:,0] = k  #add value image index for build_target()

        return torch.stack(img, 0), torch.cat(label, 0), path, shapes, torch.cat(value,0)
