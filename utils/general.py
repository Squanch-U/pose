...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    """
    #prediction=Tensor:{1,57375,62}
    if nc is None:
        nc = prediction.shape[2] - 5  if not kpt_label else prediction.shape[2] - 56 # number of classes #nc:6
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            if not kpt_label:
                i,j=(x[:,5:]>conf_thres).nonzreo(as_tuple=False).T
                x=torch.cat((box[i],x[i,j+5,None],j[:,None].float()),1)
            else:
                #x={Tensor:(20,62)}
                kpts=x[:,5+nc:]
