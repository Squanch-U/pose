...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes,value) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            #train_out tensor: {list:4}  Tensor:(1,3,120,120,62)  Tensor:(1,3,60,60,62) Tenosr:(1,3,30,30,62) Tensor(1,3,15,15,62)
            #out tensor: (1,57375,62)
            if flip_test:
            ...
            # Compute loss
            if compute_loss:
                loss += compute_loss([x.float() for x in train_out], targets,value)[1][:3]  # box, obj, cls

            # Run NMS
            if kpt_label:
                num_points = (targets.shape[1]//2 - 1)
                #num_points=(targets.shape[1]-6)//2
        ...
        for si, pred in enumerate(out):
            ...
            # Append to text file
            if save_txt:
                gn = torch.tensor(shapes[si][0])[[1, 0, 1, 0]]  # normalization gain whwh  # gn: tensor([3840,2160,3840,2160])

                for xyxy in predn.tolist():
                    xywh = (xyxy2xywh(torch.tensor(xyxy[:4]).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                    cls = float(xyxy[5:6][0])
                    conf=float(xyxy[4:5][0])
                    xyxy=np.array(xyxy)
                    xyxy[6::2] /=shapes[si][0][0]
                    xyxy[5::2]/=shapes[si][0][1]
                    save_conf=True
                    #line = (cls, *xywh, conf) if save_conf else (cls, *xywh,*xyxy[6:])  # label format
                    line = (cls,*xyxy[6::3]) if save_conf else (cls, *xywh,*xyxy[6:])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/cxh/Documents/研究-姿态检测/edgeai-yolov5-yolo-pose/runs/train/exp48/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='data/coco_sleep.yaml', help='*.data path')
    ...
    parser.add_argument('--kpt-label', default=True, help='Whether kpt-label is enabled or not')
